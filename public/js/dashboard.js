...
// Crea el formulario de publicación notificaciones con toast
  ...
  document.getElementById('btnPublicar').addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        mostrarToastDashboard(data.mensaje || 'Error al subir publicación.', 'error');
      }
    } catch (err) {
      console.error('Error al subir publicación:', err);
      mostrarToastDashboard('Hubo un error al subir la imagen.', 'error');
    }
  });
}
...
// CARGAR MIS PUBLICACIONES CON DENUNCIAS
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (userData && sidebarName) {
    try {
      const user = JSON.parse(userData);
      sidebarName.textContent = `${user.nombre} ${user.apellido}`;
    } catch (e) {
      console.error('❌ Error al mostrar nombre:', e);
    }
  }

  document.getElementById('publicaciones').addEventListener('submit', async (e) => {
    if (!e.target.matches('form.form-comentario')) return;
    e.preventDefault();

    const form = e.target;
    const input = form.querySelector('input[name="contenido"]');
    const contenido = input.value.trim();
    const publicacionId = form.getAttribute('data-comentario');
    ...
    try {
      ...
      if (res.ok) {
        ...
        const error = await res.json();
        alert(error.mensaje || 'Error al comentar');
      }
    } catch (err) {
  ...
  cargarConteo('/api/seguidos/count', 'countSiguiendo');
  cargarConteo('/api/publicaciones/count', 'countPublicaciones');
...
//BUSCAR POR TAGS
document.getElementById('formBusquedaDashboard').addEventListener('submit', async (e) => {
  e.preventDefault();
  ...
  if (!query) {
    cargarMisPublicaciones(); // 🔄 Recargar publicaciones si el input está vacío
    return;
  }
  ...
  // 👇 Nueva lógica: si empieza con #, es búsqueda por tag
  const isTagSearch = query.startsWith('#');
  const url = isTagSearch
    ? `/api/buscardashboard?tag=${encodeURIComponent(query.slice(1))}`
    : `/api/buscardashboard?query=${encodeURIComponent(query)}`;
