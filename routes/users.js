...
//Aceptar o rechazar solicitud de amistad - NUEVA SEGUIMIENTOS:

// router.post('/api/users/friend-request/:id/:accion', authMiddleware, (req, res) => {
//     const receptorId = req.usuario.id; // El usuario logueado que acepta/rechaza
//     const solicitudId = req.params.id; // El ID de la solicitud de amistad
//     const accion = req.params.accion;   // 'accept' o 'reject'
...
//         // 1. Verificar y obtener la solicitud pendiente
//         const sqlGet = `
...
//             // 2. Actualizar el estado de la solicitud en solicitudes_amistad
//             const sqlUpdate = `UPDATE solicitudes_amistad SET estado = ? WHERE id = ?`;
...
//                 // 3. Lógica específica para cuando se ACEPTA la solicitud
//                 if (accion === 'accept') {
//                     // Insertar registros bidireccionales en la tabla `seguimientos`
//                     // (Solicitante sigue a Receptor y Receptor sigue a Solicitante)
//                     const insertFollowsSql = `INSERT INTO seguimientos (seguidor_id, seguido_id) VALUES (?, ?), (?, ?)`;
//                     pool.query(insertFollowsSql, [
//                         solicitud.solicitante_id, solicitud.receptor_id, // Solicitante sigue a Receptor
//                         solicitud.receptor_id, solicitud.solicitante_id  // Receptor sigue a Solicitante
//                     ], (err3, followResult) => {
//                         if (err3) {
//                             // Este error podría ocurrir si ya existía una relación,
//                             // aunque la lógica de `sqlCheck` en `/api/friend-request` debería prevenirlo.
//                             return pool.rollback(() => {
...
//                         // 4. Crear una notificación para el solicitante original (amistad aceptada)
...
//                                 // No hacemos rollback por un fallo en la notificación secundaria,
//                                 // la amistad ya está establecida.
//                             }

//                             // Confirmar la transacción si todo ha ido bien (aceptar)
...
//                     // Solo se actualiza el estado de la solicitud en solicitudes_amistad
//                     // Puedes añadir aquí una notificación específica para el rechazo si lo deseas
//                     // Notificación en tiempo real (si aplica para rechazos)
//                     notifyRequestResponse(req.app.get('io'), solicitud.solicitante_id, {
//                       responderNombre: req.usuario.nombre,
//                       estado: nuevoEstado, // 'rechazada'
//                     });

//                     // Confirmar la transacción si todo ha ido bien (rechazar)
//                     pool.commit(errCommit => {
//                         if (errCommit) {
//                             return pool.rollback(() => {
